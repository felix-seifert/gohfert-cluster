...
KUBECTL_CONFIG_AFTER_CLUSTER_DEPLOYMENT := $(ANSIBLE_COLLECTIONS_SUBDIR)/techno_tim/k3s_ansible/kubeconfig
LOCAL_KUBECTL_CONFIG_DIR := ~/.kube
LOCAL_KUBECTL_CONFIG := $(LOCAL_KUBECTL_CONFIG_DIR)/config
...
copy-kubectl-config: $(LOCAL_KUBECTL_CONFIG)

$(LOCAL_KUBECTL_CONFIG): $(KUBECTL_CONFIG_AFTER_CLUSTER_DEPLOYMENT)
	@mkdir -p $(LOCAL_KUBECTL_CONFIG_DIR)
	cp $(KUBECTL_CONFIG_AFTER_CLUSTER_DEPLOYMENT) $(LOCAL_KUBECTL_CONFIG)
...
help:
	...
	@echo "  copy-kubectl-config        Configure local kubectl config by copying kubectl config generated by"
